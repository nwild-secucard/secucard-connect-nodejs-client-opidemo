...
function SecucardClient(config) {
    ...
    this.heartbeatTimer = setInterval(function() {
        ...
        // check last heartbeat not too long ago
        if ( _secucard.lastHeartbeat !== null && ((_secucard.lastHeartbeat +_secucard.config.heartbeat_alive_timeout) < (new Date().getTime() / 1000))) {
            _secucard.log.warn("last successfull heartbeat too long ago");
            _secucard.stomp.disconnect();
        }
...
SecucardClient.prototype.sendHeartbeat = function() {
    ...
    _secucard.rpcStomp('sessionsRefresh', {
        ...
        _secucard.lastHeartbeat = new Date().getTime() / 1000;
    });
};
...
SecucardClient.prototype.sendDeviceRequest = function(requestXml, callback) {
    ...
    // execite session refresh with refreshInterval 10 seconds longer than we plan to send our heartbeat
    var data = { xml: requestXml, xmlb64: new Buffer(requestXml).toString('base64') };
